...
输入框: 帮我看看这个爬虫为啥老是断            ← 点 ✨ 或 Ctrl+Shift+Enter
        ↓
候选框: ✨ 提示词候选   模型：deepseek-v4-flash · DeepSeek
        1  【目标】定位爬虫频繁断连的根因…
        2  【目标】给出爬虫稳定性的可落地改造方案…
        3  【目标】先诊断再修复，并附验证证据…
        ↑↓ 切换 · 1/2/3 采纳 · Enter 采纳当前 · ←→ 重新生成 · Esc 关闭
```
...
- **生成约束**：只做结构化补全，禁止编造文件/报错/需求，缺信息用 `<待确认：…>` 占位；
  原样保留 `@引用`、`/命令`、路径、报错原文。
...
| `↑` / `↓` | 切换候选 |
| `1`–`9` | 直接采纳第 N 个候选（模型列表态下为选模型） |
| `Enter` | 采纳当前高亮候选 |
| `←` / `→` | 换一个角度重新生成 |
| `Esc` | 关闭候选框（从候选框进入模型列表时，退回候选） |
| 点模型名 | 打开模型列表，换模型重新生成 |
...
{ "text": "草稿", "provider": "custom:tr", "model": "deepseek-v4-flash",
  "sessionId": "…", "count": 3, "direction": "可选：重写角度" }
→ 200 { "candidates": ["候选1", "候选2", "候选3"], "provider": "…", "model": "…" }
